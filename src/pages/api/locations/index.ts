...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createLocation() {
    ...
    const body = { ...req.body };
    if (body?.amenity?.length > 0) {
      const create_amenity = body.amenity;
      body.amenity = {
    ...
    if (body?.traffic_info?.length > 0) {
      const create_traffic_info = body.traffic_info;
      body.traffic_info = {
        create: create_traffic_info,
      };
    } else {
      delete body.traffic_info;
    }
