...
import { getAmenities, deleteAmenityById } from 'apiSdk/amenities';
import { AmenityInterface } from 'interfaces/amenity';

type ColumnType = ColumnDef<AmenityInterface, unknown>;

interface AmenityListPageProps {
...
export function AmenityListPage(props: AmenityListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AmenityInterface>>(
    () => `/amenities?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: AmenityInterface) => {
    if (hasAccess('amenity', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/amenities/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/amenities/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('amenity', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/amenities/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('amenity', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="amenity" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Amenities
        </Text>
        {hasAccess('amenity', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/amenities/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
